...
struct compareLTSet
{
  constexpr bool operator() (const T1& a_x, const T2& a_y) const noexcept
    {
      stc::CompareLT<T, N, Impl> lexCompClass;
      return std::lexicographical_compare(a_x.begin(), a_x.end(),
                                          a_y.begin(), a_y.end(),
                                          lexCompClass);
    }
};
                                                                           ...
                                                                           const bool & a_onlyBiggest)
{
  ...
  // make set operations a little easier, and specify lexicographic comparison
  using compareLTLocal = stc::CompareLT<Real, CH_LOCAL_CLASS_DATA, stc::DefaultImpl>;
  using localSet = std::set<IndexTM<Real,CH_LOCAL_CLASS_DATA>, compareLTLocal>;
    ...
    for (VoFIterator vofit(curIVS,curEBGraph); vofit.ok(); ++vofit)
    {
      ...
      if (setAllConnectedVoFsNew(totalVolFrac,curEBCellFABInfo,curBox,curEBGraph,curEBISBox,vof,vof,curNum))
      {
        ...
        if (totalVolFrac > 0.0)
        {
          ...
          IndexTM<Real,CH_LOCAL_CLASS_DATA> cur{
            (Real)myProcID,(Real)boxIndex,(Real)curNum,totalVol};
  ...
  std::set<localSet, compareLTSet<localSet,
                                  localSet,
                                  Real,
                                  CH_LOCAL_CLASS_DATA,
                                  stc::DefaultImpl > > subClasses;
...
#if CH_MPI
  {
    ...
    int curCount = 0;
    for (std::set<localSet >::iterator it = subClasses.begin();
         it != subClasses.end();
         ++it)
    {
    ...
    for (std::set<localSet >::iterator it = subClasses.begin();
         ...
         ++it)
    {
      for (localSet::iterator it2 = it->begin();
           it2 != it->end();
           ++it2)
      {
  ...
  std::list<localSet > finalClasses;

  std::list<localSet >::iterator it = curClasses.begin();
  while (it != curClasses.end())
  {
    ...
    std::list<localSet >::iterator it2 = curClasses.begin();
    while (it2 != curClasses.end())
    {
      const localSet& scanSet = *it2;

      localSet intersection;
      compareLTLocal lexComp;
      
      std::set_intersection(curSet.begin(),curSet.end(),
      ...
      if (!intersection.empty())
      {
        ...
        std::list<localSet >::iterator it2Next = it2;
        ++it2Next;
  ...
  std::map<IndexTM<Real,CH_LOCAL_CLASS_DATA-1>,
           IndexTM<Real,CH_GLOBAL_CLASS_DATA>,
           stc::CompareLT<Real,CH_LOCAL_CLASS_DATA-1,stc::DefaultImpl> >
    componentMap;

  for (std::list<localSet >::const_iterator it = finalClasses.begin();
       it != finalClasses.end();
       ++it, count++)
  {
    ...
    Real volume = 0.0;
    for (localSet::const_iterator si = it->begin();
         si != it->end();
         ++si)
    {
         ...
         ++si)
    {
      ...
      IndexTM<Real,CH_GLOBAL_CLASS_DATA> value{(Real)count,volume};
  ...
  for (DataIterator dit = coarGrids.dataIterator(); dit.ok(); ++dit)
  {
    ...
    for (VoFIterator vofit(curIVS,curEBGraph); vofit.ok(); ++vofit)
    {
      ...
      IndexTM<Real,CH_LOCAL_CLASS_DATA-1> key{
        (Real)curProcID,(Real)curBoxIndex,(Real)curComponent};
