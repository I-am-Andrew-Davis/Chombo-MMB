  ...
  for (int iedge = 0; iedge < m_msh->edges.edge.size(); iedge++)
  {
    ...
    */
    ...
    if (m_printdebug)
    {
      ...
      pout() << "  Along edge from " << v0 << " to " << RealVect(d+v0) << ":" << endl;
      pout() << curcell << " --> ";
    }
...
void STLExplorer::FindCellsInTriangles()
{
  ...
  for (int itri = 0; itri < m_msh->triangles.corners.size(); itri++)
  {
    ...
    //int idir=0; // just 1 iteration
    {
      ...
      CH_START(tunique);
      // remove duplicates
      stc::CompareEQ<int, SpaceDim> eqop;
      vector<IntVect>::iterator it = std::unique( cells.begin(), cells.end(), eqop);
      ...
      if ( tris.size() % 2 == 1 ) // odd number of intersections
      {
        ...
        else
        {
          ...
          for (int i=1; i<pts.size(); i++)
            areAllPtsEqual = areAllPtsEqual && (RealVect(pts[i]-pts[0]).vectorLength() < m_msh->tol);
          ...
          else
          {
            ...
            for (int i=0; i<tris.size(); i++)
              edgeintmap[i] = make_pair(RealVect(pts[i]-node0).vectorLength() , i);
                                    ...
                                    const int& triangle)
{
  if (SpaceDim==3)
  {
    ...
    Real dnormal = RealVect(point-projection).vectorLength();
