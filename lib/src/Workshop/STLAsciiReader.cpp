...
STLAsciiReader::STLAsciiReader(const string& a_filename)
{
  ...
  {
    MayDay::Abort("STLAsciiReader - unable to open file");
  }
                               ...
                               const int offset)
{
  ...
  CH_TIME("STLAsciiReader::ReadData");
  // CH_TIMERS("STLAsciiReader::ReadData");
  ...
  while (a_file.good())
  {
    ...
    if (RealVect(verts[0]-verts[1]).vectorLength() < m_stlmesh->tol || \
        RealVect(verts[1]-verts[2]).vectorLength() < m_stlmesh->tol || \
        RealVect(verts[2]-verts[0]).vectorLength() < m_stlmesh->tol)
    {
