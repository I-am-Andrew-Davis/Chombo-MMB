                                ...
                                const Box& a_box) const
{
  ...
  Box xbox = a_box;
  xbox.grow(2);
  if (m_hasDomain) xbox &= m_domain;  // enforce bounds if given
  FluxBox X(xbox, SpaceDim);

  CH_assert(m_volInterval.size() == SpaceDim); // Is this ever not true?
  ...
  if(m_hasDomain)
    { // this should calculate the same thing, but 1 sided where needed
      m_util.deconvolveFace(X, X);
    }
  else
    {
  ...
  computeMetricTermProductAverage(a_volFlux, X, a_Nt, SpaceDim,
                                  X, a_box, true,
                                  Interval(0,0),
                                  Interval(0,a_volFlux.nComp()-1),
                                  0, &m_domain);
}
  ...
  for (int faceDir=0; faceDir<SpaceDim; faceDir++)
    {
      ...
      Box thisDirBox = a_box;
      thisDirBox.surroundingNodes(faceDir);
...
/// NtJ evaluated at locations Xi in mapped space
void
NewFourthOrderCoordSys::pointwiseNt(FArrayBox& a_NtJ,
                                    const FArrayBox& a_Xi,
                                    const Box& a_box) const
{
  Vector<Real> NtJ(getNumN());
  MD_BOXLOOP(a_box, i)
    {
      RealVect mappedLoc(D_DECL6(a_Xi[MD_IX(i, 0)],
                                 a_Xi[MD_IX(i, 1)],
                                 a_Xi[MD_IX(i, 2)],
                                 a_Xi[MD_IX(i, 3)],
                                 a_Xi[MD_IX(i, 4)],
                                 a_Xi[MD_IX(i, 5)]));
      pointwiseNt(mappedLoc, NtJ);
      for (int c = 0, c_end = getNumN(); c != c_end; ++c)
        {
          a_NtJ[MD_IX(i, c)] = NtJ[c];
        }
    }
}
...
/** This general implementation supports computing an average of a product where
    ...
    \param[in]  a_fluxSpaceContiguous
                        -1 - The flux is a just a vector of which we need
                             SpaceDim components.  So although the stride in
                             space is 1, do not try to advance by SpaceDim to
                             get to the first variable.  ***WARNING*** this has
                             only been tested for 2nd-order.
                         0 - Indexing in space is not contiguous in the flux
                             dyad.  Figure out the stride from the total number
                             of components in the dyad (assumed to be
                             nvar*SpaceDim)
                         1 - Indexing in space is contiguous in the flux dyad
                        (default 1)
                        >1 - Use this explicit stride
  ...
  const ProblemDomain *const a_problemDomainPtr) const
{
  ...
  bool useCenterGradF = true;  // If can use a centered difference everywhere
  if (a_fourthOrder)
    {
      ...
      if (a_fluxSpaceContiguous == -1)
        {
          MayDay::Error("You need to validate the additional fourth-order "
                        "terms to support a dot product between a row of N^T "
                        "and SpaceDim terms in some vector");
        }
    }
  else
    {
  ...
  switch (a_fluxSpaceContiguous)
    ...
    case 1:
    default:
      FSpaceStride = a_fluxSpaceContiguous;
        break;
    }
...
//--First part is easy -- dot product of F and N
      ...
      FORT_POINTFDOTN(CHF_FRA(thisProductDir),
                      ...
                      CHF_CONST_INT(a_fluxSpaceContiguous),
...
//**FIXME Workaround for 1-D to avoid errors, assuming tangent
// gradients are zero for 1-D, have not verified this works for
// non-Cartesian grids (SEP)
#if CH_SPACEDIM > 1
      if (a_fourthOrder)
        {
                             ...
                             int a_s, int a_d, int a_d1) const
{
  ...
  if (a_s == 1) polarity *= -1;
  if (a_d == 1) polarity *= -1;
  return polarity*getNMatrixEntry(a_Xi, a_s, a_d, a_d1, r, c);

#elif CH_SPACEDIM == 3
  const int ra = (a_s + 1) % SpaceDim;
  const int rb = (a_s + 2) % SpaceDim;
  const int ca = (a_d + 1) % SpaceDim;
  const int cb = (a_d + 2) % SpaceDim;
  const int r0 = std::min(ra, rb);
  const int r1 = std::max(ra, rb);
  const int c0 = std::min(ca, cb);
  const int c1 = std::max(ca, cb);
  if (a_s == 1) polarity *= -1;
  if (a_d == 1) polarity *= -1;

  const Real B00 = getNMatrixEntry(a_Xi, a_s, a_d, a_d1, r0, c0);
  const Real B01 = getNMatrixEntry(a_Xi, a_s, a_d, a_d1, r0, c1);
  const Real B10 = getNMatrixEntry(a_Xi, a_s, a_d, a_d1, r1, c0);
  const Real B11 = getNMatrixEntry(a_Xi, a_s, a_d, a_d1, r1, c1);
  return polarity*0.5*(B00*B11 - B01*B10);

#else
  // This is the older 3D approach.  Can be used for Dim > 3?
  MayDay::Error("Provide support for SpaceDim > 3");
...
void
NewFourthOrderCoordSys::pointwiseNt(const RealVect& a_Xi, Vector<Real>& a_NtJ)
  const
{
  CH_assert(a_NtJ.size() >= getNumN());
  // this will look slightly odd
  // Calling getN() gives values for script N, however setting d' out
  // of bounds actually gives N^T / (D-1)
  // as noted in getN(), this is ineffecient and should be revised at some point
  for (int row=0; row<SpaceDim; ++row)
    {
      for (int col=0; col<SpaceDim; ++col)
        {
          a_NtJ[getNcomponent(col, row)] =
            getN(a_Xi, col, row, -1) * (SpaceDim - 1);
        }
    }
}
...
void
NewFourthOrderCoordSys::setProblemDomain( const ProblemDomain& a_domain) const
{
  m_util.define(a_domain, m_dx[0]); // fix me, assues uniform cell size
  m_hasDomain = true;
  m_domain = a_domain;
}
