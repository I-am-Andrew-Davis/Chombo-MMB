...
int MultiBlockCoordSys::whichBlock(const std::string& a_name) const
{
  for (int idxBlk = 0, idxBlk_end = numBlocks(); idxBlk != idxBlk_end; ++idxBlk)
    {
      if (a_name == blockInfo(idxBlk).m_name) return idxBlk;
    }
  return -1;
}
...
const ProblemDomain&
MultiBlockCoordSys::problemDomain(const Box& a_box) const
{
  const int idxBlk = whichBlock(a_box);
  return blockInfo(idxBlk).m_domain;
}

// Returns a problem domain for a block number a_idxBlk
const ProblemDomain&
MultiBlockCoordSys::problemDomain(const int a_idxBlk) const
{
  return blockInfo(a_idxBlk).m_domain;
}
...
ProblemDomain
MultiBlockCoordSys::levelDomain() const
{
  CH_assert(m_gotMappingBlocks);
  const int numBlk = m_mappingBlocks.size();
  CH_assert(numBlk > 0);
  Box domainBox = m_mappingBlocks[0];
  for (int idxBlk = 1; idxBlk != numBlk; ++idxBlk)
    {
      domainBox.minBox(m_mappingBlocks[idxBlk]);
    }
  return ProblemDomain(domainBox);
}
                                              ...
                                              int a_nSrc) const
{
  ...
  if (a_nDst == a_nSrc)
  ...
  else
    {
      ...
      // This is poorly named, and not general
      // But it assumes prior knowledge of the destination block
...
Real
MultiBlockCoordSys::distance(const RealVect&            a_srcX,
...
{
  const RealVect dstX = a_extraDisplX.transformBack(
    cachedXCoord(a_dstCell, a_dstBlk, a_dstXi, a_xCache));
  
  return stc::mag(dstX - a_srcX);
}
...
void
MultiBlockCoordSys::setCache()
{
  m_blockInfo.resize(numBlocks());
  for (int idxBlk = 0, idxBlk_end = numBlocks(); idxBlk != idxBlk_end; ++idxBlk)
    {
      ProblemDomain domain(m_mappingBlocks[idxBlk]);
      for (auto dir : EachDir)
        {
          for (auto side : EachSide)
            {
              //**FIXME - same for MAPPED and CONFORMAL?
              if (boundary(idxBlk, dir, side).isInterface())
                {
                  domain.setAsConnected(dir, side);
                }
              //**FIXME This may be the constructor and verbosity is not yet set
              //**FIXME make verbosity static
              if (m_verbosity > 1)
                {
                  const BlockBoundary& bb = boundary(idxBlk, dir, side);
                  pout() << "Boundary block " << idxBlk << ", dir " << dir
                         << ", side " << side << " is of BCtype "
                         << bb.type();
                  if (bb.isInterface())
                    {
                      pout() << " neighbor " << bb.neighbor();
                    }
                  pout() << std::endl;
                }
            }
        }
      BlockInfo& blockInfo = m_blockInfo[idxBlk];
      blockInfo.m_domain = domain;
      blockInfo.m_name = m_coordSysVect[idxBlk]->name(idxBlk);
    }
}
