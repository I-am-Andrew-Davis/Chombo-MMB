...
/*--------------------------------------------------------------------*/
//  Weak construction with known patters
/** \param[in]  a_rMotions
 *                      All remote motion patterns for this processor
 *  This is the same as the previous define but the calling routine
 *  is now responsible for figuring out all the data relations and
 *  maybe doing it a bit more efficiently.
 *//*-----------------------------------------------------------------*/
void
RemoteCopier::
define(const Vector<RemoteMotion>& a_rMotions)
{
  clear();
  m_isDefined = true;
  buffersAllocated = false;

  const int lclProc = procID();

  for (const RemoteMotion& rMotion : a_rMotions)
    {
      // Receive
      if (!rMotion.m_lclRcv.isEmpty())
        {
          // Make sure we have src and dst data
          CH_assert(!rMotion.m_lclStoDidx.isNull());
          CH_assert(!rMotion.m_rmtStoDidx.isNull());
          
          MotionItem* item = new (s_motionItemPool.getPtr())
            MotionItem(rMotion.m_rmtStoDidx, rMotion.m_lclStoDidx,
                       rMotion.m_rmtSnd, rMotion.m_lclRcv);
          if (item == nullptr)
            {
              MayDay::Error("Out of Memory in RemoteCopier::define");
            }
          if (rMotion.m_rmtProc == lclProc)  // Local move
            {
              m_localMotionPlan.push_back(item);
            }
          else
            {
              item->procID = rMotion.m_rmtProc;
              m_toMotionPlan.push_back(item);
            }
        }
      // Send (only relevant in parallel, all 
#ifdef CH_MPI
      if (!rMotion.m_lclSnd.isEmpty())
        {
          // Make sure we have src and dst data
          CH_assert(!rMotion.m_lclStoDidx.isNull());
          CH_assert(!rMotion.m_rmtStoDidx.isNull());

          if (rMotion.m_rmtProc != lclProc)  // If local motion, do not send to
                                             // self
            {
              MotionItem* item = new (s_motionItemPool.getPtr())
                MotionItem(rMotion.m_lclStoDidx, rMotion.m_rmtStoDidx,
                           rMotion.m_lclSnd, rMotion.m_rmtRcv);
              if (item == nullptr)
                {
                  MayDay::Error("Out of Memory in RemoteCopier::define");
                }
