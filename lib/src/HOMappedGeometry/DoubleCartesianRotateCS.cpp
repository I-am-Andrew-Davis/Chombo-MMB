          ...
          for (int srcDim = 0; srcDim < SpaceDim; srcDim++)
            {
              ...
              BlockBoundary& bb = boundary(srcBlock, srcDim, side);
              const int blockSideSign = 2*srcIV[dimToOther] - 1;
              if (signToOther == blockSideSign)
                {
                  // Periodic displacement (unit jump)
                  bb.definePhysicalTransformation(
                    RigidTransformation(
                      RealVect_zero,
                      RealVect((-signToOther)*RealVect_basis(dimToOther))));
                }
              else
                {
                  // Next to each other
