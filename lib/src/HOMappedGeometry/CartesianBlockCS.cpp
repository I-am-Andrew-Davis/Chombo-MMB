...
{
  RealVect scale = m_dx*m_blockBox.size();
  RealVect x = a_Xi - RealVect(m_blockIndex)*scale;
  return x;
}
...
{
  RealVect scale = m_dx*m_blockBox.size();  // Half of domain
  // Full domain is 2*scale
  RealVect xi = a_x + RealVect(m_blockIndex)*scale;
  for (int idir = 0; idir < SpaceDim; idir++)
    {
      switch (m_blockIndex[idir])
        {
        case 0: // valid x[idir] in 0:1*scale, valid a_Xi[idir] in 0:1*scale
          if (a_x[idir] > 1.5*scale[idir])
            { // wrap around
              xi[idir] -= 2.0*scale[idir];  // Shift by domain size
            }
          break;
        case 1: // valid x[idir] in 1*scale:2*scale, valid a_Xi[idir] in
                // 2*scale:3*scale
          if (a_x[idir] < 0.5*scale[idir])
            { // wrap around
              xi[idir] += 2.0*scale[idir];  // Shift by domain size
            }
          break;
