      ...
      if (!m_forwardEuler)
        {
          ...
          const Real dtRatio = 1./(Real)(getCoarserLevel()->refRatio());
          AMRLevelMappedCons* coarserPtr = getCoarserLevel();
          m_levelGridMetricsPtr->timeIntermediate(
            ...
            m_data.getJU(), // don't worry, not used due to false on next line
            false);
...
//            coarseUPtr = &(getCoarserLevel()->m_data.getU());
      ...
      m_levelGridMetricsPtr->preRegrid(a_base_level,
                                       m_grids,
                                       nullptr,  //**FIXME coarse grid needed
                                       coarseUPtr,
                                       *UPtr,
                                       JUNew);
  ...
  if (m_initialAverage)
    {
      for (DataIterator dit = m_levelGridMetricsPtr->getDataIterator();
           dit.ok(); ++dit)
        {
          ...
          const ProblemDomain& domain = m_coordSys->problemDomain(box);
          box.grow(1);
          box &= domain;
  ...
  for (DataIterator dit = m_levelGridMetricsPtr->getDataIterator(); dit.ok();
       ++dit)
    {
      ...
      fourthOrderCellProd(JUFab, UFab, JFab, interiorBox,
                          m_coordSys->problemDomain(interiorBox),
                          true);
    }
