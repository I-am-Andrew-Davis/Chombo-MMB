 ...
 *  \param[in]  a_numExtraBlockGhost
 *                      Only relevant to mapped multiblock geometries.
 *                      The number of ghost cells to be filled outside
 *                      block boundaries.  They are filled by
 *                      mulitblock exchange.  Note that extra-blocks
 *                      ghosts are filled according to the maximum,
 *                      max(a_numValidGhost, a_numExtraBlockGhost).
 *                      This setting is useful if a multi-block
 *                      exchange is needed but not a regular exchange:
 *                      a_numValidGhost = 0
 *                      a_numInvalidGhost = -1
 *                      a_numExtraBlockGhost = 3
 *//*-----------------------------------------------------------------*/

LevelData<FArrayBox>&
MappedLevelData::getU(const int a_numValidGhost,
                      const int a_numInvalidGhost,
                      const int a_numExtraBlockGhost)
{
  CH_assert(m_defined);
  CH_assert(a_numValidGhost >= 0 && a_numValidGhost <= m_numGhostU);
  CH_assert(a_numInvalidGhost >= -1 && a_numInvalidGhost <= m_numGhostU);
#ifndef NDEBUG
  if (m_LGMP->isMultiBlock())
    {
      CH_assert(a_numExtraBlockGhost >= 0 &&
                a_numExtraBlockGhost <= m_numGhostU);
    }
#endif
...
//**FIXME
/* There's currently a bug where fine ghosts are filled everywhere along MMB
   boundaries.  Mostly, this just wastes effort.  However, it also means that
   if the fill of invalids is deferred, MMB exchange must be again performed on
   U.

   The fix should also address what happens when an AMR interface is
   perpendicular to an MMB interface (like a cross).  Currently, the MMB stencil
   shrinks away from the AMR boundary (coarse-to-fine interpolation is used to
   fill some cells where fine-to-fine interpolation should be used instead).
*/
  bool fillInvalidCells = false;

  if (hasCoarser() && (a_numInvalidGhost > m_invalidGhostStatusU))
    {
      fillInvalidCells = true;
      fillInvalidGhosts();
    }
...
//--Fill valid ghosts
  ...
  if (m_LGMP->isMultiBlock() &&
      (std::max(a_numValidGhost, a_numExtraBlockGhost) > m_extraBlockStatusU ||
       // Due to a bug where fill invalid is performed all along MMB interfaces.
       // See FIXME notes above in this routine.
       fillInvalidCells))
    {
