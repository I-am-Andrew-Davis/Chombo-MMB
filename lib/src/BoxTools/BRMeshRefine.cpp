                        ...
                        ) const
{
  ...
  makeBoxesParallel(boxes, (IntVectSet&)a_tags, a_pnd, a_domain, a_maxBoxSize,
                    0, a_totalBufferSize, a_level, 100, interval);
#else
  makeBoxes(boxes, (IntVectSet&)a_tags, a_pnd, a_domain, a_maxBoxSize, 0,
            a_totalBufferSize, a_level);
#endif
                        ...
                        ) const
{
  ...
  else
  {
    ...
    {
      // low interval
      makeBoxes( a_mesh, tags_lo, a_pnd, a_domain, a_maxBoxSize, a_depth+1,
                 a_totalBufferSize, a_level);
    }
    if (!(tags_hi.isEmpty()))
    {
      // high interval
      makeBoxes( mesh_hi, tags_hi, a_pnd, a_domain, a_maxBoxSize, a_depth+1,
                 a_totalBufferSize, a_level);
    }
                                ...
                                ) const
{
  if (a_procInterval.size() == 1)
    {
      makeBoxes(a_mesh, a_tags, a_pnd, a_domain, a_maxBoxSize, a_depth,
                a_totalBufferSize, a_level);
      return;
    }
        ...
        if (lo_interval.contains(procID()) && !tags_lo.isEmpty())
          {
            ...
            makeBoxesParallel( a_mesh, tags_lo, a_pnd, a_domain, a_maxBoxSize,
                               a_depth+1, a_totalBufferSize, a_level, a_minSize,
                               lo_interval);
