  ...
  for (; dit.ok(); ++dit)
    {
      ...
      uint64_t hash = hashIV(loEnd, origin, fsize);
      loEnd/=fsize;
      ...
      for(bit.begin(); bit.ok(); ++bit)
      {
        ...
        if(a_domain.image(i)) // code still won't do periodic correctly yet.....(bvs)
          {
            uint64_t h = hashIV(i, origin, fsize);
...
#ifdef USE_STCVECTOR
  IntVect ghostMax = stc::max(a_ghostSrc, a_ghostDst);
  IntVect ghostMin = stc::min(a_ghostSrc, a_ghostDst);
#else
  ...
  for (; dit.ok(); ++dit)
    {
      ...
      auto hashBase = hashIV(baseBoxLo, origin, fixedBoxSize);
      ...
      for (bit.begin(); bit.ok(); ++bit)
        {
          ...
          if (a_domain.image(nbrLo)) // nbrLo set to periodic image in a_domain
            {
              auto hashNbr = hashIV(nbrLo, origin, fixedBoxSize);
              if (hashNbr == hashBase)
                {
                  if (includeSelf)
                    {
