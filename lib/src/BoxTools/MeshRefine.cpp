      ...
      if ( TopLevel+1 > a_baseLevel )
        {
          ...
          // This is formally correct, having it wrong never did make much of a
          // difference due to the way build support works.  However, it can
          // affect special derivations of makePNDs and properlyNested
          blocked_BufferSize[0] = 0;

          for (int i=a_baseLevel; i<=TopLevel; i++)
            {
              m_pnds[i].makeEmpty();
            }
          makePNDs(m_pnds, totalBufferSize, a_baseLevel, TopLevel, Domains,
                   a_OldMeshes, blocked_BufferSize);
          ...
          for ( int lvl = TopLevel ; lvl >= a_baseLevel ; lvl-- )
          {
            ...
            buildSupport(lvldomain, lvlboxes, modifiedTags[lvl], lvl);
...
void MeshRefine::buildSupport(const ProblemDomain& lvldomain,
                              Vector<Box>& lvlboxes,
                     ...
                     const Vector<int>&           a_bufferSize) const
{
  ...
  const IntVect crFactor = inRefineDirs(m_level_blockfactors[a_baseLevel]);
  const Vector<Box>& oldBaseMesh = a_oldMeshes[a_baseLevel];
  for (int idxBox = 0, idxBox_end = oldBaseMesh.size(); idxBox != idxBox_end;
       ++idxBox)
    {
      Box box = oldBaseMesh[idxBox];
      box.coarsen(crFactor);
      mesh |= box;
    }
  makePNDs(a_pnds, a_totalBufferSize, a_baseLevel, a_topLevel, a_domains, mesh,
           a_bufferSize);
