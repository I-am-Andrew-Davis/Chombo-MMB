              ...
              for (int i=0; i<CH_SPACEDIM; i++)
                {
                  ...
                  Vector<Box>& lo = getCoarseLocations(i, Side::Lo)[dC];
                  Vector<Box>& hi = getCoarseLocations(i, Side::Hi)[dC];
...

{
  ...
  incrementCoarse(a_coarseFlux, a_scale, a_coarseDataIndex, a_srcInterval,
                  a_dstInterval, a_dir, Side::Lo, a_box);
  incrementCoarse(a_coarseFlux, a_scale, a_coarseDataIndex, a_srcInterval,
                  a_dstInterval, a_dir, Side::Hi, a_box);
}
