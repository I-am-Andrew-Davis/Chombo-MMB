...
void dumpmemoryatexit()
{
  ...
  if (MemtrackRegistry::memtrackRegList().size() != 0)
    {
      // Delete all entries
      for (MemtrackRegList::const_iterator r =
             MemtrackRegistry::memtrackRegList().begin();
           r != MemtrackRegistry::memtrackRegList().end(); ++r)
        {
          delete *r;
        }
      // Clear the list
      MemtrackRegistry::memtrackRegList().clear();
    }
}
...
void ReportUnfreedMemory(ostream& a_os)
{
  ...
  if (MemtrackRegistry::memtrackRegList().size() != 0)
    {
      // a_os<<"---------------------------------\n";
      // a_os<<"Registered memory allocations\n";
      ...
      for (MemtrackRegList::const_iterator r =
           ...
           r != MemtrackRegistry::memtrackRegList().end(); ++r)
        {
          ...
          if ((*r)->bytes() != 0)
            {
              a_os << "Registered class " << (*r)->name() << ": "
                   << (*r)->bytes() << " bytes ("
                   << (*r)->mebibytes() << " Mb)\n";
              if ((*r)->onGPU())
                {
#ifdef CH_GPU
                  totalSizeGPU += (*r)->bytes();
#endif
                }
              else
                {
  ...
  {
    a_os << "---------------------------------\n";
    a_os << "Total Unfreed on GPU : "
         << totalSizeGPU << " bytes ("
         << totalSizeGPU/BYTES_PER_MEG << " Mb)\n";
  }

  a_os << "peak memory usage on GPU: "
       << peakGPU << " bytes ("
       << peakGPU/BYTES_PER_MEG << " Mb)\n";
#endif
}
  ...
  if (MemtrackRegistry::memtrackRegList().size() != 0)
    {
      for (MemtrackRegList::const_iterator r =
           ...
           r != MemtrackRegistry::memtrackRegList().end(); ++r)
        {
          ...
          if ((*r)->bytes() != 0)
            {
              sprintf(temp, "Registered class %-35s %12ld b  %11.4f Mb  "
                      "peak=%11.4f\n",
                      (*r)->name(),
                      (*r)->bytes(),
                      (*r)->mebibytes(),
...
#if CH_GPU
  sprintf(temp, "%-52s %12ld b  %11.4f Mb  peak=%11.4f\n",
          "Total allocated on GPU:",
          totalSizeGPU, (Real)totalSizeGPU/(Real)BYTES_PER_MEG,
          (Real)peakGPU/(Real)BYTES_PER_MEG);
  a_os << temp;
#endif
}
...
// =========== MemtrackRegistry constructor definitions ================

MemtrackRegistry::MemtrackRegistry()
  :m_bytes(0),
   m_peak(0),
   m_bytesBuffered(0),
   m_peakBuffered(0),
   m_device(Device::CPU)
{
  strncpy(m_name, "Unknown", g_MemtrackRegNameSize);
  m_name[g_MemtrackRegNameSize-1] = '\0';
  memtrackRegList().push_back(this);
}

MemtrackRegistry::MemtrackRegistry(const char*  a_name,
  ...
  :m_bytes(0),
   ...
   m_device(a_device)
{
  strncpy(m_name, a_name, g_MemtrackRegNameSize);
  m_name[g_MemtrackRegNameSize-1] = '\0';
  memtrackRegList().push_back(this);
}
  
MemtrackRegistry::MemtrackRegistry(const std::string& a_name,
                                   const Device       a_device)
  :m_bytes(0),
   m_peak(0),
   m_bytesBuffered(0),
   ...
   m_device(a_device)
{
  strncpy(m_name, a_name.c_str(), g_MemtrackRegNameSize);
  m_name[g_MemtrackRegNameSize-1] = '\0';
  memtrackRegList().push_back(this);
}
...
MemtrackRegistry::~MemtrackRegistry()
{
  // On construction, entries add themselves to the list.  But destruction
  // only occurs by deleting the list.
}

//  Get a list of registry entries
//  If the was a non-local static, it would be unordered dynamic
//  initialized (since MemtrackRegistry appears as a static in class
//  templates) making it impossible to initialize before it is
//  used.  Placing it as a local variable in a function solve the
//  initialization problem.
//  This is also thread safe in C++11
MemtrackRegList&
MemtrackRegistry::memtrackRegList()
{
  static MemtrackRegList s_memtrackRegList;
  return s_memtrackRegList;
}

// =========== MemtrackRegistry member definitions =====================
