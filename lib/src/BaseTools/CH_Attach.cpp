...
#define useemacs 1
#if useemacs
        const int numCharWritten = snprintf(buf, 1200, "emacs -geometry 80x27 -title %s -display %s --eval '(progn (gdb \" gdb -i=mi -q -f %s --pid %d\") )'",
                title, display, binaryName, proc);
        //sprintf(buf,"emacs -geometry 80x27 -title %s -display %s --eval '(progn (lldb \" lldb -p %d\") )'",
        //title, display, proc);
#else
        const int numCharWritten = snprintf(buf, 1200, "xterm -bg white -fg black -fa Monospace -fs 14 -e \"gdb -q -f %s --pid %d\" ", binaryName, proc);
#endif
        // If following fails, increase size of 'buf'
        assert(numCharWritten > 0 && numCharWritten < 1200);
        printf("%s\n",buf);
        f = popen(buf,"w");
      }
    else
      {
        const int numCharWritten = snprintf(buf, 1200, "gdb -q  %s %d", binaryName, proc);
        // If following fails, increase size of 'buf'
        assert(numCharWritten > 0 && numCharWritten < 1200);
        f=popen(buf, "w");
        fputs("where\ncall DebugCont()\ndetach\nquit\n", f);
