...
//
// Return number of parameter names for a prefix
//

int
ParmParse::countOfPrefix (const char* prefix) const
{
    const int len = std::strlen(prefix);
    int cnt = 0;
    for (PP_ListIterator<PP_entry*> li(table); li; ++li)
        if (std::strncmp(prefix, li()->defname.c_str(), len) == 0)
           ++cnt;
    return cnt;
}
