                                       ...
                                       const Interval&         a_intvl) const
{
  ...
  // A simple copy in this case of the initial solution
  if (a_timeInterpCoeff == 0. && a_stage == 0)
    {
      const int intvLength = a_intvl.size();
      for (DataIterator dit = a_U.dataIterator(); dit.ok(); ++dit)
        {
          a_U[dit].copy(m_taylorCoeffs[dit], 0, 0, intvLength);
        }
      return;
    }

  // Real rinv = 1. / Real(m_refineCoarse);
  const Real rinv = a_dtRatio;
  std::array<Real, 4> intermCoeffs;
