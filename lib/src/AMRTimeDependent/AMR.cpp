...
//-----------------------------------------------------------------------
void AMR::setFinestLevel(const int a_level)
{
  m_finest_level = a_level;
  AMRLevel::s_finest_level = a_level;
}
//-----------------------------------------------------------------------
                                    ...
                                    int                         a_proper_nest)
{
  ...
  setFinestLevel(a_amr_grids.size() - 1);
                          ...
                          bool        a_regrid)
{
  ...
  // final thing to do -- call initialGrid, postInitialGrid, and initialData on
  // undefined empty levels just in case there are setup things which need to be
  // done there
  // (DFM 2/27/02)
  for (int level = m_finest_level + 1; level <= m_max_level; ++level)
    {
  ...
  for (int level = m_max_level; level > m_finest_level; --level)
    {
      m_amrlevels[level]->postInitialGrid(true);
    }
  for (int level = m_finest_level + 1; level <= m_max_level; ++level)
  ...
  // update the grid if requested
  // this allows for new regions to be tagged and levels to be added
  if (a_regrid)
    {
      int baseLevel = 0;
      regrid(baseLevel);
  ...
  for ( ; (m_cur_step < a_max_step) &&
        ...
        ++m_cur_step, m_cur_time += old_dt_base)
    {
      ...
      if ((m_plot_period > 0.0) &&
        ...
        {
          writePlotFile();
          int numPlotPeriods = (int)((m_cur_time+m_plot_period)/m_plot_period);
          m_next_plot_time = m_plot_period*numPlotPeriods;
        }
  ...
  if (m_useSubcycling)
    {
      ...
      {
        // advance this level
        m_amrlevels[a_level]->setSubcycleParams(m_reduction_factor[a_level],
                                                a_stepsLeft,
                                                a_coarseTimeBoundary,
                                                (a_stepsLeft <= 0));
  ...
  if (m_use_meshrefine)
    {
      ...
      setFinestLevel(Min(new_finest_level, m_max_level));
...
void AMR::makeBaseLevelMesh(Vector<Box>& a_grids) const
{
  ...
  std::pair<int, const Box*> domains = m_amrlevels[0]->blockDomainVector();

  if (m_max_base_grid_size == 0)
    {
      // define base level to be single grid per domain
      a_grids.resize(domains.first);
      for (int idxDom = 0, idxDom_end = domains.first; idxDom != idxDom_end;
           ++idxDom)
        {
          a_grids[idxDom] = (domains.second)[idxDom];
        }
    }
  else
    {
      ...
      for (int idxDom = 0, idxDom_end = domains.first; idxDom != idxDom_end;
           ++idxDom)
        {
          ...
          // side first coarsen to enforce the blocking factor
          Box domain = coarsen((domains.second)[idxDom], m_blockFactor);
          if (refine(domain, m_blockFactor) != (domains.second)[idxDom])
            {
              MayDay::Error("level 0 problem domain not coarsenable by "
                            "blocking factor");
            }
          makeBoxesInDomain(a_grids, (domains.second)[idxDom]);
        }  // Loop over domains
    }
}
//-----------------------------------------------------------------------
...
//-----------------------------------------------------------------------
void AMR::makeBoxesInDomain(Vector<Box>& a_grids, const Box& a_domain) const
{
  if (m_max_base_grid_size == 0)
    {
      a_grids.push_back(a_domain);
    }
  else
    {
      // chop base level up into grids of no more than m_max_grid_size on a
      // side first coarsen to enforce the blocking factor
      Box domain = coarsen(a_domain, m_blockFactor);
      if (refine(domain, m_blockFactor) != a_domain)
        {
          MayDay::Error("Domain not coarsenable by blocking factor");
        }

      const int max_grid_size = m_max_base_grid_size / m_blockFactor;
      if (max_grid_size <= 0)
        {
          MayDay::Error("Max base grid size must be > block factor");
        }
      
      const IntVect domain_size = domain.size();
      // Divide by max_grid_size and round up
      const IntVect num_grids = (domain_size - 1)/max_grid_size + 1;
      // Divide by num_grids and round up.  This is the largest box size, <=
      // max_grid_size.
      const IntVect base_size = (domain_size - 1)/num_grids + 1;
      // Number of boxes that have 1 less than maximum
      const IntVect num_subsz = num_grids*base_size - domain_size;
      // Number of boxes that have size base_size
      const IntVect num_regsz = num_grids - num_subsz;

      Box b(IntVect_zero, num_grids - IntVect_unit);
      const IntVect& domain_lo = domain.smallEnd();
      MD_BOXLOOP(b, i)
        {
          const IntVect iv = MD_GETIV(i);
          const IntVect lo = domain_lo + iv*base_size -
            stc::max(0, iv - num_regsz);
          const IntVect hi = lo + base_size - 1 -
            stc::min(stc::max(0, iv + 1 - num_regsz), 1);
          Box grid(lo,hi);
  ...
  if (m_use_meshrefine)
    {
      for (int top_level = 0;
           ...
           ++top_level)
        {
          ...
          setFinestLevel(m_mesh_refine_ptr->regrid(new_grids,
                                                   tags,
                                                   0,
                                                   top_level,
                                                   old_grids));
  ...
  else
    {
      ...
      new_grids = m_amr_grids;
      setFinestLevel(m_amr_grids.size() - 1);
    }
