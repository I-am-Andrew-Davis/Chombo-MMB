                           ...
                           Real                        a_weight)
{
  ...
  if (m_forwardEuler)
    ...
    {
      fillGhostsRK4(a_U, dtOrig, timeFineOrig, m_evalCount-1,
  ...
  if (m_hasCoarser)
    {
      ...
      const Real dtRatio = a_dt/dtCoarse;
      m_patcher.fillRK4Intermediate(a_U, alpha, dtRatio, a_stage, 0, 0, m_numCons);
    }
  // Exchange all the ghost cell data between grids at this level
  exchangeGhosts(a_U);
  // dummy statement in order to get around gdb bug
