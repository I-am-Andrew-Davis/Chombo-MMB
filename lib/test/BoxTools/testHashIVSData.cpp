...
static const char *pgmname = "testHashIVSData" ;
...
//--Experimentation
  ...
  using HashLayout = HashIVS::Layout<HashIVS::LocalProcMap<stc::Vector<int, 3>>,
                                     unsigned, int, unsigned, 8, 16, 16>;

  using HashData = HashIVS::Data<Real, HashIVS::dynamic, HashLayout,
                                 4, 4, 4, 1>;

  HashData ivd{};
  ivd(HashLayout::iixvec_type{ 1, 1, 1}, 1) = 1.5;
  ivd(HashLayout::iixvec_type{ 1, 1, 1}, 0) = 0.5;
  ivd(HashLayout::iixvec_type{ 0, 1, 1}, 0) = -0.5;
  ivd(HashLayout::iixvec_type{ 0, 1, 1}, 3) = 4.5;
  // ivl |= HashLayout::IIxVec{ 1, 1, 1 };
  // ivl |= HashLayout::IIxVec{ 4, 1, 1 };
  // ivl |= HashLayout::IIxVec{ 0, 1, 1 };
  // ivl |= HashLayout::IIxVec{ -1, 0, 0 };
  auto& ivl = ivd.layout();
  ivl.forEachIV([]
  ...
  // std::cout << "Sz : " << ivd.hashmap_size() << std::endl;
...
//--Set 2

  {
    int stat2 = 0;
    {
      // using HashLayout = HashIVSLayout<LocalProcMap<stc::Vector<int, 3>>,
      //                                  unsigned, int, unsigned, 16, 16, 16>;
      // HashLayout ivl;
