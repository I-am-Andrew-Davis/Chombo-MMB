  ...
  }
  {
...

    {
      ...
      if(setIVS.size() != count)
        MayDay::Error("std::set<IntVect> count is different than "
                      "TreeIntVectSet");
      for (it.begin(); it.ok(); ++it)
        {
    ...
    // std::less is defined for IntVect but not when stc::Vector is used
    // to replace IndexTM.  Here we test the more general case where the
    // comparison function object is provided.
#ifdef USE_STCVECTOR
    {
      std::set<IntVect, stc::CompareLT<int, SpaceDim> > setIVS;
      for (it.begin(); it.ok(); ++it)
        {
          setIVS.insert(it());
        }
